...
interface SidebarProps {
  ...
  onNavigateToTab?: (tab: ActiveTab, query?: string) => void;
  onLogout?: () => void;
}
...
export default function Sidebar({ 
  ...
  const themes = [
    ...
    { id: "dark", name: "Dark", dotClass: "bg-[#05070A] border-[#1A2433]" }
  ] as const;
    ...
    <>
      ...
      >
        ...
        <div className="p-6 flex-1 overflow-y-auto max-h-[calc(100vh-180px)]">
          ...
          <div className="space-y-6">
            {navSections.map((section, idx) => (
              ...
              >
                <h3 className="px-4 text-[9px] font-mono text-neutral-400 font-bold uppercase tracking-widest mb-3 flex items-center justify-between">
                   {section.title}
                   <div className="h-[1px] flex-1 ml-3 bg-gradient-to-r from-neutral-200 to-transparent premium-border" />
                </h3>
                    ...
                    return (
                      <div key={item.id}>
                        <button
                          ...
                          className={`relative w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ease-out group cursor-pointer leading-normal overflow-hidden hover:translate-x-1 ${
                            IsSelected
                              ? "premium-sidebar-active font-medium shadow-sm"
                        ...
                        >
                          ...
                          {IsSelected && (
                            <span 
                              className="absolute left-0 top-0 bottom-0 w-[3px] shadow-[0_0_8px_var(--theme-accent-text)]"
                              style={{ backgroundColor: "var(--theme-accent-text)" }}
                            />
                          )}

                          {/* Hover Highlight (Non-Active) */}
                          <div className={`absolute inset-0 transition-opacity duration-200 pointer-events-none ${IsSelected ? "opacity-0" : "opacity-0 group-hover:opacity-100 bg-neutral-950/5"}`}></div>

                          <Icon className={`w-4 h-4 shrink-0 transition-all duration-200 group-hover:scale-110 z-10 ${
                            IsSelected ? "text-[var(--theme-sidebar-active-text)]" : "text-neutral-400 group-hover:text-neutral-600"
                          }`} />
                          <span className="text-xs font-bold flex-1 z-10 transition-transform duration-200">{item.name}</span>
                          {hasSub && (
                            <ChevronDown className={`w-3.5 h-3.5 shrink-0 text-neutral-400 transition-transform z-10 ${isExpanded ? "rotate-180" : ""}`} />
                          )}
                        </button>
        ...
        <div className="flex flex-col">
          ...
          {/* Ask AI Assistant Mini Bar */}
          <div className="p-5 border-t border-neutral-200/60 premium-border premium-bg-sub/30">
            <button 
              onClick={() => {
                if (onNavigateToTab) {
              ...
              className="w-full relative flex items-center bg-white border border-neutral-200 rounded-xl py-2.5 pl-9 pr-3 text-[11px] font-medium text-neutral-400 hover:border-neutral-400 hover:text-neutral-600 transition-all cursor-pointer shadow-sm text-left"
            >
              <MessageSquareCode className="w-3.5 h-3.5 absolute left-3 text-neutral-400" />
              Ask AI Assistant...
            </button>
          </div>
